...
export function TrainingLogPage() {
  ...
  const [history, setHistory] = useState<
    { date: string; exercises: { id: string; name: string; sets: SetLog[] }[] }[]
  >([]);
  ...
  const completedSetsCount = workout.exercises.reduce(
    (sum, ex) => sum + ex.sets_data.filter((s) => s.completed).length,
    0,
  );
  ...
  const fetchActivePlan = useCallback(async () => {
    ...
    const { data: logs } = await supabase
      .from("training_logs")
      .select("id, date, sets_completed, training_exercises(name)")
      .eq("user_id", USER_ID)
    ...
    if (logs) {
      ...
      for (const log of logs as unknown as {
        ...
        training_exercises: { name: string } | null;
      }[]) {
        const arr = grouped.get(log.date) ?? [];
        arr.push({
          id: log.id,
          name: log.training_exercises?.name ?? "?",
  ...
  const deleteLog = async (logId: string) => {
    ...
    setHistory((prev) =>
      prev
        .map((day) => ({ ...day, exercises: day.exercises.filter((ex) => ex.id !== logId) }))
            ...
            {summary.records > 0 && (
              <div className="flex items-center gap-1.5 rounded-full bg-neutral-800 px-3 py-1.5">
                ...
                <span className="text-sm">
                  {summary.records} Rekord{summary.records > 1 ? "e" : ""}
                </span>
              </div>
            )}
          </div>
    ...
    return (
      <div className="flex h-[100dvh] flex-col bg-black overflow-hidden">
        ...
        <div className="flex shrink-0 items-center justify-between px-4 py-3">
          <button
            onClick={workout.goToPrevExercise}
            disabled={workout.currentExIdx === 0}
          ...
          {workout.timerActive || workout.restTimer > 0 ? (
            <>
              <p className="text-[5rem] font-bold leading-none tracking-tight tabular-nums">
                {String(Math.floor(workout.restTimer / 60)).padStart(2, "0")}:
                {String(workout.restTimer % 60).padStart(2, "0")}
              </p>
              <p className="mt-1 text-sm text-neutral-500">
                {String(Math.floor(workout.restDuration / 60)).padStart(2, "0")}:
                {String(workout.restDuration % 60).padStart(2, "0")}
              </p>
              <div className="mt-6 flex items-center gap-8">
                <button
                  onClick={() => workout.adjustTimer(-15)}
        ...
        {currentEx && (
          <div className="rounded-t-3xl bg-neutral-900 px-4 pb-4 pt-4 space-y-3 overflow-y-auto max-h-[55dvh]">
              ...
              <div>
                ...
                {currentEx.previous_log && (
                  <p className="ml-4 mt-1 text-xs text-neutral-600">
                    Vorher:{" "}
                    {currentEx.previous_log.map((s) => `${s.weight_kg}kg ×${s.reps}`).join(", ")}
                  </p>
                )}
              </div>
            </div>
                ...
                <div key={setIdx} className="flex items-center gap-3">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    {!currentEx.saved && currentEx.sets_data.length > 1 && (
                      <button
                        onClick={() => workout.removeSet(workout.currentExIdx, setIdx)}
                    ...
                    <button
                      onClick={() =>
                        !currentEx.saved && workout.toggleSetComplete(workout.currentExIdx, setIdx)
                      }
                      disabled={currentEx.saved}
            ...
            <div className="flex items-center gap-3 pt-1">
              {!currentEx.saved && (
                <button
                  onClick={() => workout.addSet(workout.currentExIdx)}
              ...
              {workout.currentExIdx < workout.exercises.length - 1 && (
                <button
                  onClick={workout.goToNextExercise}
            ...
            <div key={day.date} className="rounded-xl bg-card p-4 space-y-2">
              ...
              {day.exercises.map((ex) => (
                <div key={ex.id} className="ml-6 flex items-center gap-2 text-xs text-neutral-400">
                  ...
                  <button
                    onClick={() => deleteLog(ex.id)}
                    className="shrink-0 text-neutral-700 hover:text-red-500"
                  >
